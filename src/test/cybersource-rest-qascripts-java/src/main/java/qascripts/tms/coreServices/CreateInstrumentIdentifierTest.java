...
public class CreateInstrumentIdentifierTest {
	...
	public TmsV1InstrumentidentifiersPost200Response process() throws Exception {
		...
		File inputFile = new File(System.getProperty("user.dir")
				+ "\\src\\main\\java\\CSV_Files\\TMS\\CoreServices\\CreateInstrumentIdentifier.csv");
		...
		while ((columns = csvReader.readNext()) != null) {
			...
			merchantInitiatedTransaction.previousTransactionId(columns[1]);
			...
			try {
				...
				prevTransactionId = response.getProcessingInformation().getAuthorizationOptions().getInitiator()
						.getMerchantInitiatedTransaction().getPreviousTransactionId();
				if (!response.getState().getValue().equals(ConstantUtility.ACTIVE_STATE)) {
					CsvUtility.writingToCsv(columns[0], ConstantUtility.CREATE_INSTRUMENT_IDENTIFIER,
					...
					Assert.assertEquals(columns[1], prevTransactionId);
					CsvUtility.writingToCsv(columns[0], ConstantUtility.CREATE_INSTRUMENT_IDENTIFIER, ConstantUtility.ASSERTION_PASS,
				...
				CsvUtility.writingToCsv(columns[0], ConstantUtility.CREATE_INSTRUMENT_IDENTIFIER, ConstantUtility.FAIL, reason, null,
						apiClient.getResponseCode(), Boolean.TRUE);
			} catch (AssertionError e) {
				CsvUtility.writingToCsv(columns[0], ConstantUtility.CREATE_INSTRUMENT_IDENTIFIER, ConstantUtility.ASSERTION_FAILED,
						columns[3], response.getId(), apiClient.getResponseCode(), Boolean.TRUE);
			} catch (Exception e) {
				CsvUtility.writingToCsv(columns[0], ConstantUtility.CREATE_INSTRUMENT_IDENTIFIER, ConstantUtility.FAIL,
