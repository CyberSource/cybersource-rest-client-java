...
public class VoidCaptureTest {
	...
	public  PtsV2PaymentsVoidsPost201Response process() throws Exception {
		...
		File inputFile = new File(System.getProperty("user.dir")
				+ "\\src\\main\\java\\CSV_Files\\Payments\\CoreServices\\VoidCapture.csv");
		CSVReader csvReader = new CSVReader(new FileReader(inputFile));
		String[] columns = null;
		csvReader.readNext();

		VoidCaptureRequest request = null;
		...
		while ((columns = csvReader.readNext()) != null) {
			...
			try {
				...
				response = voidApi.voidCapture(request, columns[1]);
				if (!response.getStatus().getValue().equals(ConstantUtility.VOIDED_STATUS)) {
					CsvUtility.writingToCsv(columns[0], ConstantUtility.VOID_CAPTURE,
							ConstantUtility.ASSERTION_FAILED, ConstantUtility.NOT_VOIDED_STATUS, response.getId(),
					...
					CsvUtility.writingToCsv(columns[0], ConstantUtility.VOID_CAPTURE, ConstantUtility.ASSERTION_PASS,
				...
				CsvUtility.writingToCsv(columns[0], ConstantUtility.VOID_CAPTURE, ConstantUtility.FAIL, reason, null,
						apiClient.getResponseCode(), Boolean.TRUE);
			} catch (Exception e) {
				CsvUtility.writingToCsv(columns[0], ConstantUtility.VOID_CAPTURE, ConstantUtility.FAIL,
