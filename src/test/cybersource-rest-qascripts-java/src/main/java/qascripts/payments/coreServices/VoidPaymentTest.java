...
public class VoidPaymentTest {
	...
	public PtsV2PaymentsVoidsPost201Response process() throws Exception {
		...
		File inputFile = new File(System.getProperty("user.dir")
				+ "\\src\\main\\java\\CSV_Files\\Payments\\CoreServices\\VoidPayment.csv");
		...
		while ((columns = csvReader.readNext()) != null) {
			...
			try {
				...
				response = voidApi.voidPayment(request, columns[1]);
				
				if (!response.getStatus().getValue().equals(ConstantUtility.VOIDED_STATUS)) {
					CsvUtility.writingToCsv(columns[0], ConstantUtility.VOID_PAYMENT, ConstantUtility.ASSERTION_FAILED,
					...
					CsvUtility.writingToCsv(columns[0], ConstantUtility.VOID_PAYMENT, ConstantUtility.ASSERTION_PASS,
				...
				CsvUtility.writingToCsv(columns[0], ConstantUtility.VOID_PAYMENT, ConstantUtility.FAIL, reason, null,
						apiClient.getResponseCode(), Boolean.TRUE);
			} catch (Exception e) {
				CsvUtility.writingToCsv(columns[0], ConstantUtility.VOID_PAYMENT, ConstantUtility.FAIL, e.getMessage(),
