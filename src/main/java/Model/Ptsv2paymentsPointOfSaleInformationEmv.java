...
public class Ptsv2paymentsPointOfSaleInformationEmv {
  ...
  **/
  @ApiModelProperty(value = "Number assigned to a specific card when two or more cards are associated with the same primary account number.  This value enables issuers to distinguish among multiple cards that are linked to the same account.  This value can also act as a tracking tool when reissuing cards.   When this value is available, it is provided by the chip reader.   When the chip reader does not provide this value, do not include this field in your request.  When sequence number is not provided via this API field, the value is extracted from EMV tag 5F34 for Visa and Mastercard transactions. To enable this feature please call support.  **Note** Card present information about EMV applies only to credit card processing and PIN debit processing.  All other card present information applies only to credit card processing.   PIN debit processing is available only on CyberSource through VisaNet and FDC Nashville Global.  #### Used by Authorization: Optional PIN Debit processing: Optional  #### GPX  This field only supports transactions from the following card types: - Visa - Mastercard - AMEX - Discover - Diners - JCB - Union Pay International ")
  ...
  **/
  ...
  public String toString() {
    ...
    if (tags != null) sb.append("    tags: ").append(SENSITIVE_FIELD_PATTERN.matcher("tags").matches() ? "[REDACTED]" : toIndentedString(tags)).append("\n");
    ...
    if (fallback != null) sb.append("    fallback: ").append(SENSITIVE_FIELD_PATTERN.matcher("fallback").matches() ? "[REDACTED]" : toIndentedString(fallback)).append("\n");
    if (fallbackCondition != null) sb.append("    fallbackCondition: ").append(SENSITIVE_FIELD_PATTERN.matcher("fallbackCondition").matches() ? "[REDACTED]" : toIndentedString(fallbackCondition)).append("\n");
    if (isRepeat != null) sb.append("    isRepeat: ").append(SENSITIVE_FIELD_PATTERN.matcher("isRepeat").matches() ? "[REDACTED]" : toIndentedString(isRepeat)).append("\n");
    sb.append("}");
    return sb.toString();
  }
